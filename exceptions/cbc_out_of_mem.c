...
static struct cbc_Exception_Class_struct _cbc_OutOfMemoryError___class__ = {
...
static cbc_var _cbc_OutOfMemoryError___getclass__(cbc_var id){
	switch((size_t)id){
		case 0:
		case CBC_OUT_OF_MEMORY_ERROR_ID:
		case CBC_OBJECT_ID: return &_cbc_OutOfMemoryError___class__;
		default: cbc_throw(cbc_CastError___new__(CBC_OUT_OF_MEMORY_ERROR_ID, id)); break;
	}
	return NULL;
}

cbc_var cbc_OutOfMemoryError___sysinit__(cbc_OutOfMemoryError self){
	self->__getclass__ = &_cbc_OutOfMemoryError___getclass__;
	return self;
}
