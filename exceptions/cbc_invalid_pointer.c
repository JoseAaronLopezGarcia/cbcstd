...
static struct cbc_Exception_Class_struct _cbc_InvalidPointer___class__ = {
...
static cbc_var _cbc_InvalidPointer___getclass__(cbc_var id){
	switch((size_t)id){
		case 0:
		case CBC_INVALID_POINTER_ID:
		case CBC_OBJECT_ID: return &_cbc_InvalidPointer___class__;
		default: cbc_throw(cbc_CastError___new__(CBC_INVALID_POINTER_ID, id)); break;
	}
	return NULL;
}

cbc_var cbc_InvalidPointer___sysinit__(cbc_InvalidPointer self){
	self->__getclass__ = &_cbc_InvalidPointer___getclass__;
	return self;
}
